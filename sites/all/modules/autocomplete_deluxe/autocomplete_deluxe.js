...
(function($) {
  ...
  Drupal.autocomplete_deluxe.Widget.prototype.init = function(settings) {
    ...
    $.ui.autocomplete.prototype._renderItem = function( ul, item) {
      var element = $('<a>' + item.label + '</a>');
      var t = jQuery(element).find('.reference-autocomplete').html();
      if (this.term != "" && t) {
        var escapedValue = Drupal.autocomplete_deluxe.escapeRegex( this.term );
        var re = new RegExp('()*""' + escapedValue + '""|' + escapedValue + '()*', 'gi');
        t = t.replace(re,"<span class='autocomplete-deluxe-highlight-char'>$&</span>");
      }
      element.find('.reference-autocomplete').html(t);
      return $( "<li></li>" )
